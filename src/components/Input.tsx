...
const Input = ({handleChange, checkGuess, startTimer, timeLeft, inputPlaceholder, inputValue}: InputProps) => {
    ...
    return (
        <>
            <input ref={gameInProgress} disabled={timeLeft === 0} onChange={handleChange} placeholder={inputPlaceholder} onKeyDown={checkGuess} value={inputValue == 0 && inputPlaceholder != "Minutes" ? "00" : inputValue} className="input m-2 w-25" />
        </>
    )
}
